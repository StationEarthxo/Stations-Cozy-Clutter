...
The `run` task starts RuneLite in developer mode with Stations Cozy Clutter loaded.

## Building

1. Enable **Stations Cozy Clutter** in RuneLite's plugin list.
2. Open the coloured-block **Stations Cozy Clutter** button in RuneLite's sidebar.
3. Search for an object's name and click its 3D preview to select it.
4. Hold **Shift** and right-click a ground tile.
5. Open **Stations Cozy Clutter** and choose **Place**.

You can also hold **Shift**, right-click existing scenery, and choose **Copy to Stations Cozy Clutter**.
...
From the ground-tile **Stations Cozy Clutter** menu, use **Export Tilepack** to copy a `WB1:` code. Use **Import Tilepack** after copying someone else's code. Imported objects stay anchored to their saved world coordinates.
